#!/usr/bin/env python3
"""
run_kucoin.py — automatic v01T execution on KuCoin Futures.

    python run_kucoin.py                    # paper, no exchange contact
    python run_kucoin.py --preflight        # verify the account is ready
    python run_kucoin.py --mode dry_run     # real data, orders logged not sent
    python run_kucoin.py --mode live        # real orders (needs V01T_LIVE)
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="v01T automatic execution on KuCoin")
    ...
    print("=" * 78)
    print(f"v01T AUTOMATIC EXECUTION — KUCOIN FUTURES   mode={a.mode}   "
          f"symbol={a.symbol}")
    print("=" * 78)
    ...
    if client:
        ...
        print(f"  passphrase: {'set' if client.api_passphrase else 'MISSING'}")
    print()
        ...
        for i in range(spec.HV_MIN_CLOSES, len(closes)):
            ...
            if r.get("executed"):
                ...
                print(f"  [{n:3d}] squeeze @ {e['entry_price']:>11,.2f}  "
                      f"{e['contracts_per_leg']} contracts/leg  "
                      f"(${e['notional_per_leg']:,.0f})  "
